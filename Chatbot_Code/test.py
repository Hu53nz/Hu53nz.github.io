...
MAX_REQUESTS_BEFORE_BLOCK = 3  # Number of rate limit breaches before blocking  
LOG_FILE = "request_logs.txt"  # File to log the requests in text format  
BLOCK_DATA_FILE = "block_data.json"  # File to store blocked accounts and request counts  
USER_DATA_FILE = "user_data.json"  # File to store registered user accounts  
REQUESTS_PER_SET = 10  # Number of requests per set for aggregation  
...
if 'total_requests' not in st.session_state:  
    st.session_state.total_requests = 0  
if 'blocked_users_count' not in st.session_state:  
    st.session_state.blocked_users_count = 0  
if 'cpu_utilization_with_rl' not in st.session_state:  
    st.session_state.cpu_utilization_with_rl = []  
if 'cpu_utilization_without_rl' not in st.session_state:  
    st.session_state.cpu_utilization_without_rl = []  
...
# Load and save block data

def save_block_data():  
    block_data = {  
        "blocked_users": st.session_state.blocked_users,  
        "request_count": {user: list(times) for user, times in st.session_state.request_count.items()},  
        "rate_limit_violations": dict(st.session_state.rate_limit_violations),  
        "user_block_count": dict(st.session_state.user_block_count)  
    }  
    with open(BLOCK_DATA_FILE, "w") as file:  
...
def load_block_data():  
    if os.path.exists(BLOCK_DATA_FILE):  
        with open(BLOCK_DATA_FILE, "r") as file:  
            block_data = json.load(file)  
            st.session_state.blocked_users = block_data.get("blocked_users", {})  
            st.session_state.request_count = defaultdict(list, block_data.get("request_count", {}))  
            st.session_state.rate_limit_violations = defaultdict(int, block_data.get("rate_limit_violations", {}))  
            st.session_state.user_block_count = defaultdict(int, block_data.get("user_block_count", {}))  
    else:  
...
def log_request(user, status, message=""):  
    current_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))  
    log_message = f"{current_time} - User: {user} - Status: {status} - {message}\n"  
    with open(LOG_FILE, mode="a") as file:  
        file.write(log_message)  
...
load_block_data()  
st.title("Chatbot with Rate Limiting, DoS Protection, and User Management")
...
if st.session_state.user_account:
    st.success(f"Logged in as {st.session_state.user_account}")
    if st.button("Logout"):
        st.session_state.user_account = None
...
# Proceed with the chatbot functionality if logged in
if st.session_state.user_account:
    ...
    current_time = time.time()
    if use_rate_limiting:
        if user_account in st.session_state.blocked_users and current_time < st.session_state.blocked_users[user_account]:
            st.error("Potential DoS attack detected. Your account is temporarily blocked.")
            unblock_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(st.session_state.blocked_users[user_account]))
            log_request(user_account, "Blocked", f"Blocked until {unblock_time}")
        else:
            st.session_state.request_count[user_account] = [req for req in st.session_state.request_count[user_account] if current_time - req < 60]
            ...
            if user_input:
                ...
                for query in queries:
                    if query.strip():
                        ...
                        if len(st.session_state.request_count[user_account]) >= RATE_LIMIT_PER_MINUTE:
                            st.session_state.rate_limit_warnings[user_account] += 1
                            
                            # Handle warnings and blocking logic
                            if st.session_state.rate_limit_warnings[user_account] == 1:
                                st.warning("Rate limit exceeded. Please wait 2 seconds before sending more queries.")
                                log_request(user_account, "Rate limit exceeded", f"User input: {query.strip()}")
                            elif st.session_state.rate_limit_warnings[user_account] == 2:
                                st.warning("Rate limit exceeded again. Please wait before sending more queries.")
                                log_request(user_account, "Rate limit exceeded again", f"User input: {query.strip()}")
                            elif st.session_state.rate_limit_warnings[user_account] >= 3:
                                # Temporarily or permanently block the user
                                st.session_state.blocked_users[user_account] = current_time + BLOCK_DURATION
                                st.session_state.blocked_users_count += 1
                                st.session_state.user_block_count[user_account] += 1

                                if st.session_state.user_block_count[user_account] >= 3:
                                    st.session_state.blocked_users[user_account] = float('inf')
                                    st.error("You have been permanently blocked due to repeated rate limit violations.")
                                    log_request(user_account, "Permanently Blocked", "User has been permanently blocked.")
                                else:
                                    unblock_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(st.session_state.blocked_users[user_account]))
                                    st.error(f"Potential DoS attack detected. Your account is temporarily blocked until {unblock_time}.")
                                    log_request(user_account, "Blocked", f"Blocked until {unblock_time}")
                                save_block_data()
                                break  # Exit loop immediately

                            # Skip processing the query
                            break
                        else:
                            # Process the query if within the rate limit
                            st.session_state.request_count[user_account].append(current_time)
                            response, response_time, cpu_utilization = get_openai_response(query.strip())
                            if response:
                                st.text(f"Bot: {response}")
                                ...
                                st.session_state.cpu_utilization_with_rl.append(cpu_utilization)
                                st.session_state.response_time_with_rl.append(response_time)
                            log_request(user_account, "Success", f"User input: {query.strip()}")
